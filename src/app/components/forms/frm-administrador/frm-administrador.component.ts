...
import { AdministradorService } from '../../../helpers/services/administrador.service';
...
@Component({
  ...
  templateUrl: './frm-administrador.component.html',
  styleUrl: './frm-administrador.component.scss'
})
export class FrmAdministradorComponent implements OnInit {
  titulo!: string;
  private data = inject(MAT_DIALOG_DATA);
  private builder = inject(FormBuilder);
  private srvAdmin = inject(AdministradorService);
  private readonly dialog = inject(MatDialog);
  dialogRef = inject(MatDialogRef<FrmAdministradorComponent>);
  myForm: FormGroup;
  ...
  onGuardar() {
    ...
    if (this.myForm.value.id === 0) {
      this.srvAdmin.guardarAdministrador(this.myForm.value).
        subscribe({
          complete: () => {
      ...
      this.srvAdmin.guardarAdministrador(this.myForm.value, this.myForm.value.id)
