...
export class AdministradorComponent implements AfterViewInit {
  ...
  mostrarDialog(titulo: string, datos?: TipoUsuario) {
    const dialogRef = this.dialog.open(FrmAdministradorComponent, {
      width: '50%',
      data: {
        title: titulo,
        datos: datos
  ...
  filtrar() {
    this.adminSrv.filtrarAdministrador(this.filtro)
      .subscribe({
  ...
  onEditarClick(id: number) {
    this.adminSrv.getAdministrador(id)
      .subscribe({
        next: (res) => {
          this.mostrarDialog("Editar admin", res);
  ...
  onEliminarClick(id: number) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        this.adminSrv.eliminarAdministrador(id)
          .subscribe((res: any) => {
            this.resetearFiltro();
  ...
  onNuevoClick() {
    this.mostrarDialog("Nuevo Administrador");
  }
  ...
  onImprimir() {
    ...
    this.srvImpresion.print(encabezado, cuerpo, 'Administradores', true);
  }
  
  ngAfterViewInit() {
    this.adminSrv.getAll().subscribe({
      next: (result) => {
